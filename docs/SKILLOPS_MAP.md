...
| Formato Google Sheets | `google-sheets-format-guardian` | Formatear Sheets (cabeceras, colores, bordes, freeze, anchos) y replicar el look de una hoja modelo; contraparte de `google-docs-format-guardian` para Sheets | Dominio/formato + MCP | Verde | Leer formato de hoja modelo con `readCellFormat`; usar `copyFormatting` para match exacto (bordes/fuente/wrap que `formatCells` no hace) | Google Workspace MCP + `google-workspace-editor` | `readCellFormat` de verificacion |
