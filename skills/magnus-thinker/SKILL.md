...
23. **Los angulos nuevos pueden nacer de la cadena.** No usar solo angulos predefinidos. F5A/F5B pueden generar angulos adicionales; F6 puede revelar actores o flujos invisibles; F7 puede abrir angulos por tendencias externas; F8 puede elevar una opcion por asimetria. Si un angulo nuevo cambia la decision, entra al dossier como rama real con pregunta, evidencia, hallazgo y decision.
...
34. **Subcapas recursivas reales.** Si una subcapa genera un vacio nuevo, Magnus debe crear una sub-subcapa solo para resolver ese vacio. No detenerse en el primer nivel. Cada nivel debe explicar: que se encontro, por que importa, que no prueba, que nueva pregunta abre y que decision cambia.
35. **Cadena completa = bateria amplia interna.** Cuando se active Magnus completo/F1-F13/deep research estrategico, Magnus debe correr una bateria amplia de preguntas por fase internamente. El output visible solo muestra las 1-3 preguntas decisivas por fase, pero las variables, vacios y angulos descubiertos por preguntas no mostradas tambien deben alimentar el analisis. Si no se ejecuto esa exploracion amplia, no afirmar que corrio Magnus completo.
